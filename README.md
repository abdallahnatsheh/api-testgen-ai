...
- **Auth flow support** — `--auth-url` / `--auth-payload` / `--auth-token-path` auto-logins before running tests and injects the token as Bearer — no manual token copy-paste
...
- Optional auth: static Bearer token, custom header (e.g. `X-API-Key`), or dynamic auth flow
- Includes a sample FastAPI server with JWT auth for local testing
...
├── api.py                        # Sample FastAPI server (GET /users, POST /login, GET /me — JWT auth)
...
│   └── test_api_server.py        # 29 unit tests for api.py (/login, /users, /me)
├── examples/                     # Ready-to-run test case files, organized by endpoint
│   ├── login/
│   │   ├── description.md        # API description fed to the AI
│   │   ├── tests_gemini.json     # Tests generated by Gemini (12/12 pass)
│   │   └── tests_ollama.json     # Tests generated by Ollama
│   ├── me/
│   │   ├── description.md        # Description for GET /me (auth flow aware)
│   │   └── tests_gemini.json     # Tests generated by Gemini (7/7 pass)
│   ├── users/
│   │   ├── description.md
│   │   ├── tests_gemini.json
│   │   └── tests_ollama.json
...
|----------|--------|------|-------------|
| `/users` | GET | None | Returns list of all users |
| `/users/{id}` | GET | None | Returns a single user by integer ID |
| `/login` | POST | None | Validates credentials, returns a signed JWT |
| `/me` | GET | Bearer JWT | Returns the current user's profile |

**Valid credentials:**

| Email | Password | Result |
|-------|----------|--------|
| `alice@example.com` | `alice123` | 200 + JWT token |
| `bob@example.com` | `bob123` | 200 + JWT token |
| `carol@example.com` | `carol123` | 200 + JWT token |
| `locked@example.com` | `locked123` | 200 + `{"detail": "Welcome!"}` *(known bug — should be 403)* |
...
python3 main.py --url http://localhost:8000/me --method GET \
  --bearer mytoken \
  --provider ollama --model qwen2.5-coder:7b --run

# With auth flow — auto-login before running tests
python3 main.py --url http://localhost:8000/me --method GET \
...
# With static Bearer token
python3 tester.py examples/me/tests_gemini.json http://localhost:8000 --bearer eyJ0eXAiOiJKV1Q...

# With auth flow — auto-login, extract token, inject as Bearer
